...
namespace SOAP.FunctionalTests
{
    /// <summary>
    ...
    public static class SoapHelper
    {
        ...
        public static string SendSOAPRequest(string url, string action, Dictionary<string, string> parameters, string soapAction = null, bool useSOAP12 = false)
        {
            ...
            var s = string.Format(xmlStr, action, "http://tempuri.org/", parms);
        ...
        public static string GetEnvelope(string action, Dictionary<string, string> parameters)
        {
            XmlDocument soapEnvelopeXml = new XmlDocument();
            ...
            string parms = string.Join(string.Empty, parameters.Select(kv => string.Format("<{0}>{1}</{0}>", kv.Key, kv.Value)).ToArray());
            return string.Format(xmlStr, action, "http://tempuri.org/", parms);
        }
    }
}
